    ...
    *,
    ...
    if session.geofencing_enabled:
        ...
        if distance > (session.radius_meters or 50):
            logger.warning(
                f"Check-in failed for student {student.user_id} on session {session.id}: Proximity violation ({distance:.1f}m > {session.radius_meters or 50}m)",
                extra={
                    "student_id": student.user_id,
                    "session_id": session.id,
                    "session_code": payload.session_code,
            ...
            raise HTTPException(
                status_code=400,
                detail=f"Check-in failed. You are outside the designated area ({distance:.1f}m > {session.radius_meters or 50}m)."
            )
